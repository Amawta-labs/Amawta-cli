...
import { runAmawtaAgentStreamJsonSession } from '@utils/protocol/agentStreamJsonSession'
...
describe('stream-json session interrupt (integration)', () => {
  test('interrupt aborts active turn and emits an error result, then continues next turn', async () => {
    ...
    const structured = new AmawtaAgentStructuredStdio(stdin, stdout, {
      onInterrupt: () => {
    ...
    const sessionPromise = runAmawtaAgentStreamJsonSession({
      structured,
      query,
      writeSdkLine: obj => {
