...
describe('npm runtime (Node.js)', () => {
  test('built dist/index.js runs on Node for --help-lite/--version', async () => {
    const root = mkdtempSync(join(tmpdir(), 'amawta-node-runtime-'))
    ...
    try {
      await esbuildBuild({
        ...
        sourcemap: false,
        banner: {
          js: 'import { createRequire as __amawtaCreateRequire } from "node:module";\nconst require = __amawtaCreateRequire(import.meta.url);',
        },
      ...
      expect((helpRes.stdout ?? '')).toContain('Usage: amawta')
    } finally {
