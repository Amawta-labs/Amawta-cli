...
import type { Message as AmawtaMessage } from '@query'
...
describe('TUI E2E regression (Ink render)', () => {
  ...
  test('queued Waiting… progress is replaced by Running… for same tool_use_id', async () => {
    ...
    function AutoUpdateMessagesHarness(): React.ReactNode {
      const [messages, setMessages] = useState<AmawtaMessage[]>([waiting])
  ...
  test('statusline renders when configured', async () => {
    ...
    const originalEnabled = process.env.AMAWTA_STATUSLINE_ENABLED
    const originalConfigDir = process.env.AMAWTA_CONFIG_DIR

    const homeDir = mkdtempSync(join(tmpdir(), 'amawta-statusline-home-'))
    process.env.HOME = homeDir
    process.env.USERPROFILE = homeDir
    process.env.AMAWTA_STATUSLINE_ENABLED = '1'
    process.env.AMAWTA_CONFIG_DIR = join(homeDir, '.amawta')

    mkdirSync(join(homeDir, '.amawta'), { recursive: true })
    const cmd = `${process.execPath} -e "process.stdout.write('hello-statusline')"`
    writeFileSync(
      join(homeDir, '.amawta', 'settings.json'),
    ...
    try {
      ...
      if (originalEnabled === undefined)
        delete process.env.AMAWTA_STATUSLINE_ENABLED
      else process.env.AMAWTA_STATUSLINE_ENABLED = originalEnabled
      if (originalConfigDir === undefined) delete process.env.AMAWTA_CONFIG_DIR
      else process.env.AMAWTA_CONFIG_DIR = originalConfigDir
