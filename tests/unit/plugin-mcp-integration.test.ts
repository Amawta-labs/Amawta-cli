...
describe('Plugin MCP integration (.mcp.json + plugin.json mcpServers)', () => {
  ...
  const originalExec = process.env.AMAWTA_TEST_EXEC
  ...
  beforeEach(async () => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-plugin-mcp-proj-'))
    homeDir = mkdtempSync(join(tmpdir(), 'amawta-plugin-mcp-home-'))
    process.env.AMAWTA_CONFIG_DIR = join(homeDir, '.amawta')
    process.env.MY_TOKEN = 'shh'
    process.env.AMAWTA_TEST_EXEC = process.execPath
  ...
  afterEach(async () => {
    ...
    if (originalExec === undefined) delete process.env.AMAWTA_TEST_EXEC
    else process.env.AMAWTA_TEST_EXEC = originalExec
