...
describe('JSONL session persistence (projects/*.jsonl)', () => {
  ...
  beforeEach(async () => {
    resetSessionJsonlStateForTests()
    setAmawtaAgentSessionId('704b907b-2b0f-478d-a7cb-b9fecf921913')
    configDir = mkdtempSync(join(tmpdir(), 'amawta-session-jsonl-config-'))
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-session-jsonl-project-'))
  ...
  test('writes file-history-snapshot then user/assistant records with parentUuid chaining', async () => {
    ...
    expect(lines[1].sessionId).toBe(getAmawtaAgentSessionId())
    ...
    expect(lines[2].sessionId).toBe(getAmawtaAgentSessionId())
