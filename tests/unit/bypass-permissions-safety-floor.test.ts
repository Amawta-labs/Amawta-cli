...
describe('bypassPermissions safety floor', () => {
  ...
  test('allows bypassing the safety floor via env (non-safe mode)', async () => {
    const prev = process.env.AMAWTA_BYPASS_SAFETY_FLOOR
    process.env.AMAWTA_BYPASS_SAFETY_FLOOR = '1'
    try {
      const filePath = resolve(homedir(), '.ssh', 'config')
      const result = await hasPermissionsToUseTool(
        FileWriteTool as any,
      ...
      if (prev === undefined) delete process.env.AMAWTA_BYPASS_SAFETY_FLOOR
      else process.env.AMAWTA_BYPASS_SAFETY_FLOOR = prev
    }
  })
})
