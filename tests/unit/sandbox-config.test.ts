...
import type { AmawtaSettingsFile } from '@utils/sandbox/sandboxConfig'
import {
...
describe('sandbox config (Reference CLI parity: YC1 + z34)', () => {
  ...
  beforeEach(() => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-sandbox-project-'))
    homeDir = mkdtempSync(join(tmpdir(), 'amawta-sandbox-home-'))
  })
  ...
  test('normalizes network + filesystem config from settings + permissions rules', () => {
    const settings: AmawtaSettingsFile = {
      permissions: {
    ...
    expect(runtime.filesystem.denyWrite).toContain(
      join(projectDir, '.amawta', 'settings.json'),
    )
    expect(runtime.filesystem.denyWrite).toContain(
      join(projectDir, '.amawta', 'settings.local.json'),
    )
    expect(runtime.filesystem.denyWrite).toContain(
