...
describe('log paths (messages)', () => {
  const originalLogRoot = process.env.AMAWTA_LOG_ROOT
  const originalLegacyCacheRoot = process.env.AMAWTA_LEGACY_CACHE_ROOT

  let newRoot: string
  let legacyRoot: string

  beforeEach(() => {
    newRoot = mkdtempSync(join(tmpdir(), 'amawta-log-new-'))
    legacyRoot = mkdtempSync(join(tmpdir(), 'amawta-log-legacy-'))

    process.env.AMAWTA_LOG_ROOT = newRoot
    process.env.AMAWTA_LEGACY_CACHE_ROOT = legacyRoot
  })

  afterEach(() => {
    if (originalLogRoot === undefined) {
      delete process.env.AMAWTA_LOG_ROOT
    } else {
      process.env.AMAWTA_LOG_ROOT = originalLogRoot
    }

    if (originalLegacyCacheRoot === undefined) {
      delete process.env.AMAWTA_LEGACY_CACHE_ROOT
    } else {
      process.env.AMAWTA_LEGACY_CACHE_ROOT = originalLegacyCacheRoot
    }
  ...
  test('getMessagesPath uses the new log root (AMAWTA_LOG_ROOT)', () => {
