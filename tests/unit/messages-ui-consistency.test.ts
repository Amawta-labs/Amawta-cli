...
describe('UI messages consistency (no duplicate tool rendering)', () => {
  ...
  test('reorderMessages keeps persistent progress messages for the same tool_use_id', () => {
    const toolUse = makeToolUseAssistantWithSiblings(['t1'])
    const siblings = new Set(['t1'])

    const start = makeProgress('t1', siblings, 'tool_start: R1', {
      persistHistory: true,
    })
    const end = makeProgress('t1', siblings, 'tool_end: R1', {
      persistHistory: true,
    })

    const normalized = normalizeMessages([toolUse, start, end] as any)
    const ordered = reorderMessages(normalized)
    const progress = ordered.filter(m => m.type === 'progress')
    expect(progress).toHaveLength(2)
    const texts = progress.map(p =>
      extractTag(
        String((p as any).content.message.content[0]?.text ?? ''),
        'tool-progress',
      ),
    )
    expect(texts).toEqual(['tool_start: R1', 'tool_end: R1'])
  })
