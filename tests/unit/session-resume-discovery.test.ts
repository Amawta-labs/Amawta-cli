...
import { getSessionLogFilePath } from '@utils/protocol/agentSessionLog'
import {
  listAmawtaAgentSessions,
  resolveResumeSessionIdentifier,
} from '@utils/protocol/agentSessionResume'
...
describe('resume session discovery', () => {
  ...
  beforeEach(() => {
    configDir = mkdtempSync(join(tmpdir(), 'amawta-resume-config-'))
    projectA = mkdtempSync(join(tmpdir(), 'amawta-resume-project-a-'))
    projectB = mkdtempSync(join(tmpdir(), 'amawta-resume-project-b-'))
  ...
  test('listAmawtaAgentSessions returns sorted sessions with metadata', () => {
    ...
    const sessions = listAmawtaAgentSessions({ cwd: projectA })
