...
function makeTempPackageRoot(options: { version: string }) {
  const root = mkdtempSync(join(tmpdir(), 'amawta-cli-wrapper-'))
  ...
  writeFileSync(
    ...
    JSON.stringify(
      { name: '@shareai-lab/amawta-test', version: options.version },
      null,
      2,
    ) + '\n',
    'utf8',
...
function createBuildCapableBunOnPath(dir: string) {
  mkdirSync(dir, { recursive: true })
  if (process.platform === 'win32') {
    return
  }
...
if [ "$1" = "run" ] && [ "$2" = "build:npm" ]; then
  mkdir -p dist
  cat > dist/index.js <<'EOF'
console.log("DIST_BUILT_OK", process.argv.slice(2).join(" "));
EOF
  exit 0
fi
exit 1
...
describe('cli.js wrapper (binary-first + bun fallback)', () => {
  test('--help-lite prints usage without requiring Bun', () => {
    ...
    const emptyPath = mkdtempSync(join(tmpdir(), 'amawta-empty-path-'))
    try {
      const res = runWrapper(pkg.root, ['--help-lite'], {
        PATH: emptyPath,
      })
      expect(res.status).toBe(0)
      expect(res.stdout).toContain('Usage: amawta')
  ...
  test('falls back to Bun when native binary is missing and Bun is available on PATH', () => {
    const pkg = makeTempPackageRoot({ version: '9.9.9' })
    const stubDir = mkdtempSync(join(tmpdir(), 'amawta-stub-bun-'))
    try {
      writeFileSync(
  ...
  test('builds local dist automatically when missing in source checkout', () => {
    if (process.platform === 'win32') return

    const pkg = makeTempPackageRoot({ version: '9.9.9' })
    const stubDir = mkdtempSync(join(tmpdir(), 'amawta-stub-bun-build-'))
    try {
      rmSync(join(pkg.root, 'dist'), { recursive: true, force: true })
      mkdirSync(join(pkg.root, '.git'), { recursive: true })
      mkdirSync(join(pkg.root, 'src'), { recursive: true })

      createBuildCapableBunOnPath(stubDir)

      const res = runWrapper(pkg.root, ['arg1'], {
        PATH: `${stubDir}${delimiter}${process.env.PATH || ''}`,
      })
      expect(res.status).toBe(0)
      expect(res.stdout).toContain('DIST_BUILT_OK arg1')
    } finally {
      rmSync(stubDir, { recursive: true, force: true })
      pkg.cleanup()
    }
  ...
  test('prefers native cached binary when present (non-Windows)', () => {
    ...
    const pkg = makeTempPackageRoot({ version: '9.9.9' })
    const binDir = mkdtempSync(join(tmpdir(), 'amawta-bin-cache-'))
    try {
      const platform = process.platform
      const arch = process.arch
      const cachedBinary = join(binDir, '9.9.9', `${platform}-${arch}`, 'amawta')
  ...
  test('prints guidance and exits 1 when neither binary nor Bun is available', () => {
    ...
    try {
      ...
      expect(res.stderr).toContain('Amawta CLI is not runnable')
      expect(res.stderr).toContain('AMAWTA_BINARY_BASE_URL')
    } finally {
      rmSync(emptyPath, { recursive: true, force: true })
      pkg.cleanup()
    }
