...
describe('AskUserQuestion Other text input filtering (Reference CLI parity)', () => {
  ...
  test('accepts normal printable characters (including space)', () => {
    ...
    expect(__isTextInputCharForTests('impl', {})).toBe(true)
    expect(__isTextInputCharForTests('impl', { return: true })).toBe(true)
    expect(__isTextInputCharForTests('hello', {})).toBe(true)
