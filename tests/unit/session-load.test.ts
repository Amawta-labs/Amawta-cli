...
import {
  ...
  loadAmawtaAgentSessionLogData,
  loadAmawtaAgentSessionMessages,
} from '@utils/protocol/agentSessionLoad'
import {
...
describe('session loader (projects/*.jsonl)', () => {
  ...
  beforeEach(() => {
    configDir = mkdtempSync(join(tmpdir(), 'amawta-claude-load-config-'))
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-claude-load-project-'))
  ...
  test('loads user/assistant messages from a session jsonl file', () => {
    ...
    const messages = loadAmawtaAgentSessionMessages({
      cwd: projectDir,
      sessionId,
    })
  ...
  test('loads summary/custom-title/tag metadata from session log', () => {
    ...
    const data = loadAmawtaAgentSessionLogData({ cwd: projectDir, sessionId })
  ...
  test('loads toolUseResult data from user messages with tool results', () => {
    ...
    const lines =
      [
        ...
        // User message with tool_result AND toolUseResult data (as saved by amawtaAgentSessionLog)
        JSON.stringify({
          type: 'user',
          sessionId,
          uuid: 'u2',
  ...
  test('findMostRecentAmawtaAgentSessionId picks newest jsonl by mtime', () => {
    ...
    expect(findMostRecentAmawtaAgentSessionId(projectDir)).toBe(
