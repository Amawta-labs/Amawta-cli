...
describe('--plugin-dir runtime: commands & skills discovery', () => {
  ...
  beforeEach(async () => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-plugin-dir-project-'))
    await setCwd(projectDir)

    pluginDir = join(projectDir, 'hookify')
    mkdirSync(join(pluginDir, '.amawta-plugin'), { recursive: true })
    writeFileSync(
      join(pluginDir, '.amawta-plugin', 'plugin.json'),
      JSON.stringify(
