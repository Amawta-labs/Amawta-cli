...
describe('skill marketplace (local .amawta-plugin/marketplace.json)', () => {
  ...
  beforeEach(async () => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-skill-mkt-proj-'))
    homeDir = mkdtempSync(join(tmpdir(), 'amawta-skill-mkt-home-'))
    await setCwd(projectDir)

    repoDir = join(projectDir, 'skills-repo')
    mkdirSync(join(repoDir, '.amawta-plugin'), { recursive: true })
    mkdirSync(join(repoDir, 'skills', 'pdf'), { recursive: true })
    mkdirSync(join(repoDir, 'skills', 'xlsx'), { recursive: true })

    writeFileSync(
      join(repoDir, '.amawta-plugin', 'marketplace.json'),
      JSON.stringify(
        {
          name: 'my-marketplace',
  ...
  test('add marketplace + install/uninstall skills into user dir', async () => {
    await withEnv({ AMAWTA_CONFIG_DIR: join(homeDir, '.amawta') }, async () => {
      ...
      expect(existsSync(join(homeDir, '.amawta', 'skills', 'pdf'))).toBe(true)
      expect(existsSync(join(homeDir, '.amawta', 'skills', 'xlsx'))).toBe(true)
      ...
      expect(existsSync(join(homeDir, '.amawta', 'skills', 'pdf'))).toBe(false)
      expect(existsSync(join(homeDir, '.amawta', 'skills', 'xlsx'))).toBe(false)
    })
  })

  test('project install writes to ./.amawta/skills based on agent cwd', async () => {
    await withEnv({ AMAWTA_CONFIG_DIR: join(homeDir, '.amawta') }, async () => {
      ...
      expect(install.installedSkills.sort()).toEqual(['pdf', 'xlsx'])
      expect(existsSync(join(projectDir, '.amawta', 'skills', 'pdf'))).toBe(true)
      expect(existsSync(join(projectDir, '.amawta', 'skills', 'xlsx'))).toBe(true)
    })
  })
