...
import { createDefaultToolPermissionContext } from '@amawta-types/toolPermissionContext'
...
describe('Bash permission dont-ask-again (prefix) parity', () => {
  ...
  test('prefix allow takes effect immediately in same turn after savePermission()', async () => {
    ...
    const projectDir = mkdtempSync(join(tmpdir(), 'amawta-p024-'))
  ...
  test('prefix allow persists to .amawta/settings.local.json and reloads on restart', async () => {
    if (process.platform === 'win32') return

    const originalCwd = getCwd()
    const projectDir = mkdtempSync(join(tmpdir(), 'amawta-p024-'))
    const homeDir = mkdtempSync(join(tmpdir(), 'amawta-home-'))
    await setCwd(projectDir)
