...
describe('tool permission settings (multi-source load/merge/persist)', () => {
  ...
  beforeEach(() => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-perm-project-'))
    homeDir = mkdtempSync(join(tmpdir(), 'amawta-perm-home-'))
  })
  ...
  test('loads .amawta/settings.json + .amawta/settings.local.json across sources', () => {
    writeJson(join(homeDir, '.amawta', 'settings.json'), {
      permissions: { allow: ['Bash(ls:*)'] },
    })
    writeJson(join(projectDir, '.amawta', 'settings.json'), {
      permissions: { allow: ['Bash(git:*)'] },
    })
    writeJson(join(projectDir, '.amawta', 'settings.local.json'), {
    ...
    const ctx = loadToolPermissionContextFromDisk({
      projectDir,
      homeDir,
      includeAmawtaProjectConfig: false,
    })
  ...
  test('migrates legacy .claude settings to .amawta when .amawta is missing', () => {
    ...
    const migratedPath = join(projectDir, '.amawta', 'settings.local.json')
  ...
  test('session updates are not persisted; policySettings is never persisted', () => {
    ...
    expect(existsSync(join(projectDir, '.amawta', 'settings.local.json'))).toBe(
      false,
    )
  ...
  test('persists allow rules to localSettings (.amawta/settings.local.json)', () => {
    ...
    const settingsPath = join(projectDir, '.amawta', 'settings.local.json')
