...
describe('messages normalization + reordering parity', () => {
  ...
  test('reorderMessages inserts tool_result after the latest progress for same tool_use_id', () => {
    const toolUse = makeToolUseAssistant('t1')
    const start = createProgressMessage(
      't1',
      new Set(['t1']),
      createAssistantMessage('tool_start'),
      [],
      [],
      { persistHistory: true },
    )
    const end = createProgressMessage(
      't1',
      new Set(['t1']),
      createAssistantMessage('tool_end'),
      [],
      [],
      { persistHistory: true },
    )
    ...
    const normalized = normalizeMessages([toolUse, start, end, toolResult] as any)
    const reordered = reorderMessages(normalized)

    const order = reordered.map(m => m.type)
    expect(order).toEqual(['assistant', 'progress', 'progress', 'user'])
  })
