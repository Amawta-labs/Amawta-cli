...
describe('projectInstructions (AGENTS.md discovery)', () => {
  test('findGitRoot returns null when no .git is found', () => {
    const root = mkdtempSync(join(tmpdir(), 'amawta-agents-test-'))
  ...
  test('getProjectDocMaxBytes respects AMAWTA_PROJECT_DOC_MAX_BYTES with fallback', () => {
    const original = process.env.AMAWTA_PROJECT_DOC_MAX_BYTES
    try {
      process.env.AMAWTA_PROJECT_DOC_MAX_BYTES = '1234'
      expect(getProjectDocMaxBytes()).toBe(1234)

      process.env.AMAWTA_PROJECT_DOC_MAX_BYTES = '0'
      expect(getProjectDocMaxBytes()).toBeGreaterThan(0)

      process.env.AMAWTA_PROJECT_DOC_MAX_BYTES = 'not-a-number'
      expect(getProjectDocMaxBytes()).toBeGreaterThan(0)
    } finally {
      if (original === undefined) delete process.env.AMAWTA_PROJECT_DOC_MAX_BYTES
      else process.env.AMAWTA_PROJECT_DOC_MAX_BYTES = original
    }
  })
...
describe('projectDocs (legacy CLAUDE.md fallback)', () => {
  test('getProjectDocs includes legacy CLAUDE.md when present', async () => {
    const root = mkdtempSync(join(tmpdir(), 'amawta-claude-legacy-test-'))
