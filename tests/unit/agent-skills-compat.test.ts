...
describe('Agent Skills compatibility (discovery + prompt)', () => {
  ...
  beforeEach(async () => {
    projectDir = mkdtempSync(join(tmpdir(), 'amawta-skill-proj-'))
    homeDir = mkdtempSync(join(tmpdir(), 'amawta-skill-home-'))
  ...
  test('loads .amawta/skills/<name>/SKILL.md and splits allowed-tools string', async () => {
    await withEnv(
      {
        AMAWTA_CONFIG_DIR: join(homeDir, '.amawta'),
        AMAWTA_SKILLS_STRICT: undefined,
      },
      async () => {
        const skillDir = join(projectDir, '.amawta', 'skills', 'test-skill')
  ...
  test('accepts lowercase skill.md when SKILL.md is missing', async () => {
    await withEnv(
      ...
      async () => {
        const skillDir = join(projectDir, '.amawta', 'skills', 'lower-skill')
        mkdirSync(skillDir, { recursive: true })
        writeFileSync(
          join(skillDir, 'skill.md'),
          [
  ...
  test('strict mode skips skills whose frontmatter name mismatches directory', async () => {
    await withEnv(
      { AMAWTA_CONFIG_DIR: join(homeDir, '.amawta'), AMAWTA_SKILLS_STRICT: '1' },
      async () => {
        const skillDir = join(projectDir, '.amawta', 'skills', 'dir-name')
        mkdirSync(skillDir, { recursive: true })
        writeFileSync(
          join(skillDir, 'SKILL.md'),
          [
  ...
  test('SkillTool.prompt includes skill location path when available', async () => {
    await withEnv(
      ...
      async () => {
        const skillDir = join(projectDir, '.amawta', 'skills', 'alpha')
        mkdirSync(skillDir, { recursive: true })
