...
describe('toolOutputDisplay', () => {
  test('isPackagedRuntime: AMAWTA_PACKAGED=1 forces true', async () => {
    await withEnv({ AMAWTA_PACKAGED: '1' }, async () => {
  ...
  test('isPackagedRuntime: heuristic treats bun/node as not packaged', async () => {
    await withEnv({ AMAWTA_PACKAGED: undefined }, async () => {
      await withExecPath('/usr/local/bin/bun', () => {
  ...
  test('isPackagedRuntime: heuristic treats other execPath as packaged', async () => {
    await withEnv({ AMAWTA_PACKAGED: undefined }, async () => {
      await withExecPath('/usr/local/bin/amawta', () => {
        expect(isPackagedRuntime()).toBe(true)
      })
      await withExecPath('C:\\\\amawta\\\\amawta.exe', () => {
        expect(isPackagedRuntime()).toBe(true)
      })
    })
  })
  ...
  test('maybeTruncateVerboseToolOutput: truncates in packaged runtime', async () => {
    ...
    await withEnv(
      { AMAWTA_PACKAGED: '1', AMAWTA_TOOL_OUTPUT_FULL: undefined },
      () => {
        const out = maybeTruncateVerboseToolOutput(longText)
  ...
  test('maybeTruncateVerboseToolOutput: AMAWTA_TOOL_OUTPUT_FULL disables truncation', async () => {
    const longText = Array.from({ length: 200 }, (_, i) => `L${i + 1}`).join(
      '\n',
    )
    await withEnv({ AMAWTA_PACKAGED: '1', AMAWTA_TOOL_OUTPUT_FULL: '1' }, () => {
      const out = maybeTruncateVerboseToolOutput(longText)
      expect(out.truncated).toBe(false)
      expect(out.text).toBe(longText)
    })
  ...
  test('maybeTruncateVerboseToolOutput: env overrides max lines/chars', async () => {
    ...
    await withEnv(
      {
        AMAWTA_PACKAGED: '1',
        AMAWTA_TOOL_OUTPUT_FULL: undefined,
        AMAWTA_TOOL_OUTPUT_MAX_LINES: '2',
        AMAWTA_TOOL_OUTPUT_MAX_CHARS: '3',
      },
      () => {
        const out = maybeTruncateVerboseToolOutput(longText)
        expect(out.truncated).toBe(true)
    ...
    await withEnv(
      {
        AMAWTA_PACKAGED: '1',
        AMAWTA_TOOL_OUTPUT_FULL: undefined,
        AMAWTA_TOOL_OUTPUT_MAX_LINES: 'not-a-number',
        AMAWTA_TOOL_OUTPUT_MAX_CHARS: 'not-a-number',
      },
      () => {
        const out = maybeTruncateVerboseToolOutput(longText, {
          maxLines: 2,
