...
function usage() {
  console.log(
    [
      ...
      '',
      'Env:',
      '  AMAWTA_REFERENCE_REPO=<path>    Reference repo root (alternative to --reference)',
      '',
      'Notes:',
      '- Runs offline parity checks (stdout/stderr/exit codes) against a reference repo.',
      '- Forces wrappers to skip any cached native binaries by setting AMAWTA_BIN_DIR to a temp dir.',
    ].join('\n'),
  )
}
...
async function main() {
  ...
  const referenceInput = referenceArg || process.env.AMAWTA_REFERENCE_REPO
  if (!referenceInput) {
    usage()
    console.error('')
    console.error('Error: --reference (or AMAWTA_REFERENCE_REPO) is required.')
    process.exit(1)
  }
  ...
  if (!referenceRoot || referenceRoot === newRoot) {
    console.error('Error: --reference (or AMAWTA_REFERENCE_REPO) must point to a different repo root.')
    process.exit(1)
  }
  ...
  const tmp = mkdtempSync(join(tmpdir(), 'amawta-reference-parity-'))
