...
function getPackageName() {
  return readPackageJson()?.name || 'amawta-cli'
}
...
function downloadFile(url, destPath, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(
      ...
      {
        headers: {
          'User-Agent': `${getPackageName()} postinstall`,
...
async function maybeInstallBinary() {
  if (process.env.AMAWTA_SKIP_BINARY_DOWNLOAD) {
    return
  }

  const pkg = readPackageJson()
  const version = pkg?.version
  const packageName = pkg?.name || 'amawta-cli'
  if (!version) return
  ...
  safeLog(`📦 ${packageName}: installing native binary for ${platformArch} (v${version})`)
  ...
  try {
    ...
    safeLog(`✅ ${packageName}: native binary ready at ${dest}`)
  } catch (err) {
    safeWarn(`⚠️  ${packageName}: could not download native binary (${platformArch})`)
    safeWarn(`    URL: ${url}`)
    safeWarn(
      `    Reason: ${err instanceof Error ? err.message : String(err)}`,
    )
    safeWarn(`    This is non-fatal. ${packageName} will fall back to Bun if available.`)
  }
}
...
async function postinstallNotice() {
  ...
  safeLog(
    `✅ ${packageName} installed. Commands available: amawta (legacy aliases: amawta, kwa, kd)`,
  )
  safeLog('   If shell cannot find them, reload your terminal or reinstall globally:')
  safeLog(`   npm i -g ${packageName}  (or use: npx ${packageName})`)
