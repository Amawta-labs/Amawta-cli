...
function getBinaryFilename(platform = process.platform) {
  return platform === 'win32' ? 'amawta.exe' : 'amawta'
}

function getDefaultBinBaseDir() {
  return (
    process.env.AMAWTA_BIN_DIR ||
    process.env.AMAWTA_BIN_DIR ||
    path.join(os.homedir(), '.amawta', 'bin')
  )
}
...
function getGithubReleaseBinaryAssetName(platform = process.platform, arch = process.arch) {
  ...
  return `amawta-${platform}-${arch}${ext}`
}
...
function getGithubReleaseBinaryUrl(options) {
  ...
  const repo = options?.repo ?? 'amawta'
  const tag = options?.tag ?? `v${version}`
  const baseUrl = options?.baseUrl ?? process.env.AMAWTA_BINARY_BASE_URL
