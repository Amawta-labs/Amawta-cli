...
function loadPluginFromDir(rootDir: string): SessionPlugin {
  const primaryManifestPath = join(rootDir, '.amawta-plugin', 'plugin.json')
  ...
  if (!existsSync(manifestPath)) {
    throw new Error(
      `Plugin manifest not found (expected .amawta-plugin/plugin.json or .claude-plugin/plugin.json)`,
    )
  }
