...
import { getConfigBaseDir } from '@utils/config/env'
...
function getUserAmawtaBaseDir(): string {
...
function loadPluginSkillDirectoryCommandsFromBaseDir(args: {
  ...
  const strictMode = toBoolean(process.env.AMAWTA_SKILLS_STRICT)
...
export const loadCustomCommands = memoize(
  async (): Promise<CustomCommandWithScope[]> => {
    const cwd = getCwd()
    const userAmawtaBaseDir = getUserAmawtaBaseDir()
    ...
    const projectAmawtaCommandsDir = join(cwd, '.amawta', 'commands')
    const userAmawtaCommandsDir = join(userAmawtaBaseDir, 'commands')

    const projectLegacySkillsDir = join(cwd, '.claude', 'skills')
    const userLegacySkillsDir = join(homedir(), '.claude', 'skills')
    const projectAmawtaSkillsDir = join(cwd, '.amawta', 'skills')
    const userAmawtaSkillsDir = join(userAmawtaBaseDir, 'skills')
  ...
  () => {
    ...
    const dirs = [
      ...
      join(userAmawtaBaseDir, 'commands'),
      join(cwd, '.amawta', 'commands'),
      join(homedir(), '.claude', 'skills'),
      join(cwd, '.claude', 'skills'),
      join(userAmawtaBaseDir, 'skills'),
      join(cwd, '.amawta', 'skills'),
    ]
...
export function getCustomCommandDirectories(): {
  ...
  userAmawtaCommands: string
  projectAmawtaCommands: string
  userAmawtaSkills: string
  projectAmawtaSkills: string
} {
  const userAmawtaBaseDir = getUserAmawtaBaseDir()
  return {
    ...
    userAmawtaCommands: join(userAmawtaBaseDir, 'commands'),
    projectAmawtaCommands: join(getCwd(), '.amawta', 'commands'),
    userAmawtaSkills: join(userAmawtaBaseDir, 'skills'),
    projectAmawtaSkills: join(getCwd(), '.amawta', 'skills'),
  }
}
...
export function hasCustomCommands(): boolean {
  ...
  return (
    ...
    existsSync(dirs.userAmawtaCommands) ||
    existsSync(dirs.projectAmawtaCommands) ||
    existsSync(dirs.userAmawtaSkills) ||
    existsSync(dirs.projectAmawtaSkills)
  )
}
