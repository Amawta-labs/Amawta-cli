...
export function validatePluginOrMarketplacePath(
  ...
  let filePath = abs
  if (stat.isDirectory()) {
    const marketplace = join(abs, '.amawta-plugin', 'marketplace.json')
    const plugin = join(abs, '.amawta-plugin', 'plugin.json')
        ...
        errors: [
          {
            path: 'directory',
            message:
              'No manifest found in directory. Expected .amawta-plugin/marketplace.json or .amawta-plugin/plugin.json (legacy .claude-plugin/* is also supported)',
          },
        ],
        warnings: [],
      }
