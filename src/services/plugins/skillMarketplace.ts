...
function scopeBaseDir(scope: PluginScope): string {
  if (scope === 'user') return userAmawtaDir()
...
function pluginsDir(): string {
  return join(userAmawtaDir(), 'plugins')
}
...
function installedSkillPluginsPath(): string {
  return join(userAmawtaDir(), INSTALLED_SKILL_PLUGINS_FILE)
}
...
function readMarketplaceFromDirectory(rootDir: string): MarketplaceManifest {
  ...
  if (!existsSync(marketplaceFile)) {
    throw new Error(
      `Marketplace file not found (expected .amawta-plugin/marketplace.json or .claude-plugin/marketplace.json)`,
    )
  }
...
function baseDirForInstallRecord(record: InstalledSkillPlugin): string {
  if (record.scope === 'user') return userAmawtaDir()
  const projectPath =
...
async function cacheMarketplaceToTempDir(
  ...
  if (source.source === 'file') {
    ...
    const out = join(tempDir, '.amawta-plugin')
    ensureDir(out)
