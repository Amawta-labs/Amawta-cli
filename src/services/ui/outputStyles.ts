...
function getUserConfigBaseDirs(): { claude: string; amawta: string }[] {
  const out: { claude: string; amawta: string }[] = []

  const hasAnyOverride =
    typeof process.env.CLAUDE_CONFIG_DIR === 'string' ||
    typeof process.env.AMAWTA_CONFIG_DIR === 'string'

  const claudeBase = normalizeString(process.env.CLAUDE_CONFIG_DIR)
  const amawtaBase = normalizeString(process.env.AMAWTA_CONFIG_DIR)

  if (claudeBase) out.push({ claude: resolve(claudeBase), amawta: resolve(claudeBase) })
  if (amawtaBase) out.push({ claude: resolve(amawtaBase), amawta: resolve(amawtaBase) })
  ...
  return dedupeConfigBases([
    { claude: join(homedir(), '.claude'), amawta: join(homedir(), '.claude') },
    { claude: join(homedir(), '.amawta'), amawta: join(homedir(), '.amawta') },
  ])
}

function dedupeConfigBases(
  bases: Array<{ claude: string; amawta: string }>,
): Array<{ claude: string; amawta: string }> {
  const seen = new Set<string>()
  const out: Array<{ claude: string; amawta: string }> = []
  for (const base of bases) {
    const key = `${base.claude}::${base.amawta}`
...
function findProjectSubdirs(subdir: string, cwd: string): string[] {
  ...
  while (current !== home) {
    ...
    const amawtaDir = join(current, '.amawta', subdir)
    if (existsSync(amawtaDir)) result.push(amawtaDir)
...
function loadCustomOutputStyles(options: { cwd: string }): OutputStyleDefinition[] {
  ...
  if (isSettingSourceEnabled('userSettings')) {
    ...
    for (const base of userBases) {
      for (const userBaseDir of new Set([base.claude, base.amawta])) {
