...
async function defaultGateQuery(args: {
  ...
  signal: AbortSignal
  model?: 'quick' | 'main'
  __testQueryLLMModule?: {
    queryLLM: any
    API_ERROR_MESSAGE_PREFIX: string
  }
}): Promise<string> {
  const { API_ERROR_MESSAGE_PREFIX, queryLLM } = args.__testQueryLLMModule
    ? args.__testQueryLLMModule
  ...
  try {
    ...
    const query =
      ...
      (async args =>
        defaultGateQuery({
          ...args,
          __testQueryLLMModule: params.__testQueryLLMModule,
        }))
