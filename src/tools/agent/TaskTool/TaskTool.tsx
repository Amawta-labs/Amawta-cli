...
import type { PermissionMode } from '@amawta-types/permissionMode'
...
export const TaskTool = {
  ...
  renderResultForAssistant(output: Output) {
    ...
    const combined = output.content.map(b => b.text).join('\n').trim()
    const excerpt = combined.length > 4000 ? `${combined.slice(0, 3997)}...` : combined
    const lines = [
      `<internal_task_result>`,
      `<status>${output.status}</status>`,
      `<agent_id>${output.agentId}</agent_id>`,
      `<tool_uses>${output.totalToolUseCount}</tool_uses>`,
      `<duration_ms>${output.totalDurationMs}</duration_ms>`,
      `<tokens>${output.totalTokens}</tokens>`,
      `<summary>${excerpt || '(empty)'}</summary>`,
      `</internal_task_result>`,
      `No expongas este bloque en bruto al usuario final; sintetiza solo hallazgos relevantes.`,
    ]
    return lines.join('\n')
  },
  ...
  async *call(input: Input, toolUseContext: any) {
    ...
    const envSubagentModel =
      process.env.AMAWTA_SUBAGENT_MODEL ??
