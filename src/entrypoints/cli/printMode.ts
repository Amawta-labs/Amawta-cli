...
export async function runPrintMode({
  ...
  if (normalizedPermissionPromptTool) {
    if (normalizedPermissionPromptTool !== 'stdio') {
      console.error(
        `Error: Unsupported --permission-prompt-tool "${normalizedPermissionPromptTool}". Only "stdio" is supported in Amawta right now.`,
      )
      process.exit(1)
    }
  ...
  const { amawtaMessageToSdkMessage, makeSdkInitMessage, makeSdkResultMessage } =
    await import('@utils/protocol/agentStreamJson')
  const { AmawtaAgentStructuredStdio } =
    await import('@utils/protocol/agentStructuredStdio')
  const {
    loadToolPermissionContextFromDisk,
    persistToolPermissionUpdateToDisk,
  } = await import('@utils/permissions/toolPermissionSettings')
  const { applyToolPermissionContextUpdates } =
    await import('@amawta-types/toolPermissionContext')

  const sessionIdForSdk = getAmawtaAgentSessionId()
  ...
  let toolPermissionContext = loadToolPermissionContextFromDisk({
    projectDir: cwd,
    includeAmawtaProjectConfig: true,
  ...
  const permissionTimeoutMs = (() => {
    const raw = process.env.AMAWTA_STDIO_PERMISSION_TIMEOUT_MS
  ...
  const sdkUser = amawtaMessageToSdkMessage(userMsg as any, sessionIdForSdk)
  if (sdkUser) {
    if (normalizedOutputFormat === 'stream-json') {
  ...
  try {
    for await (const m of query(baseMessages, systemPrompt, ctx, canUseTool, {
      ...
      const sdk = amawtaMessageToSdkMessage(m, sessionIdForSdk)
      if (!sdk) continue
