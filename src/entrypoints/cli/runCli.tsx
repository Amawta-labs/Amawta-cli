...
export async function runCli() {
  ...
  try {
    enableConfigs()
    try {
      const bootstrap = ensureDefaultGeminiProfiles()
      ...
      logError(`Default Gemini profile bootstrap failed: ${repairError}`)
    }
...
async function parseArgs(
  ...
  program
    ...
    .action(
      async (
        ...
        const {
          loadAmawtaAgentSessionMessages,
          findMostRecentAmawtaAgentSessionId,
        } = await import('@utils/protocol/agentSessionLoad')
        const { listAmawtaAgentSessions, resolveResumeSessionIdentifier } =
          await import('@utils/protocol/agentSessionResume')
        const { isUuid } = await import('@utils/text/uuid')
        const { setAmawtaAgentSessionId, getAmawtaAgentSessionId } =
          await import('@utils/protocol/agentSessionId')
        ...
        if (wantsContinue) {
          const latest = findMostRecentAmawtaAgentSessionId(cwd)
          if (!latest) {
            console.error('No conversation found to continue')
            process.exit(1)
          }
          initialMessages = loadAmawtaAgentSessionMessages({
            cwd,
            sessionId: latest,
          })
        ...
        if (!needsResumeSelector) {
          const effectiveSessionId = (() => {
            ...
            return getAmawtaAgentSessionId()
          })()

          setAmawtaAgentSessionId(effectiveSessionId)
        }
        ...
        if (print) {
          ...
          if (needsResumeSelector) {
            const sessions = listAmawtaAgentSessions({ cwd })
  ...
  const pluginMarketplaceCmd = pluginCmd
    .command('marketplace')
    .description(
      'Manage marketplaces (.amawta-plugin/marketplace.json; legacy .claude-plugin supported)',
    )
  ...
  const marketplaceCmd = skillsCmd
    .command('marketplace')
    .description(
      'Manage skill marketplaces (.amawta-plugin/marketplace.json; legacy .claude-plugin supported)',
    )
  ...
  skillsCmd
    ...
    .option('--cwd <cwd>', 'The current working directory', String, cwd())
    .option('--project', 'Install into this project (.amawta/...)', () => true)
    .option('--force', 'Overwrite existing installed files', () => true)
    .action(async (plugin: string, options: any) => {
      try {
        const { setCwd } = await import('@utils/state')
  ...
  skillsCmd
    ...
    .option('--cwd <cwd>', 'The current working directory', String, cwd())
    .option('--project', 'Uninstall from this project (.amawta/...)', () => true)
    .action(async (plugin: string, options: any) => {
      try {
        const { setCwd } = await import('@utils/state')
        await setCwd(options.cwd ?? cwd())
    ...
    .action(
      async (
        ...
        if (identifier !== undefined) {
          const { loadAmawtaAgentSessionMessages } =
            await import('@utils/protocol/agentSessionLoad')
          const { resolveResumeSessionIdentifier } =
            await import('@utils/protocol/agentSessionResume')
          const { setAmawtaAgentSessionId } =
            await import('@utils/protocol/agentSessionId')
          ...
          try {
            if (isLegacyNumber || isLegacyPath) {
              ...
              if (resolved.kind === 'ok') {
                setAmawtaAgentSessionId(resolved.sessionId)
                messages = loadAmawtaAgentSessionMessages({
                  cwd,
                  sessionId: resolved.sessionId,
                })
              } else if (resolved.kind === 'different_directory') {
...
async function gracefulExit(code = 0) {
  ...
  try {
    const { runSessionEndHooks } = await import('@utils/session/sessionHooks')
    const { getAmawtaAgentSessionId } =
      await import('@utils/protocol/agentSessionId')
    const { tmpdir } = await import('os')
    const { join } = await import('path')

    const sessionId = getAmawtaAgentSessionId()
    const transcriptPath = join(
      tmpdir(),
      'amawta-hooks-transcripts',
