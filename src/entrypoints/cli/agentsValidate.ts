...
function getUserConfigRoots(): string[] {
  ...
  if (hasAnyOverride) {
    return Array.from(new Set([claudeOverride, amawtaOverride].filter(Boolean))) as string[]
  }
  return [join(homedir(), '.claude'), join(homedir(), '.amawta')]
}
...
function mapClaudeModelToAmawta(model: string): string | 'inherit' {
...
function validateOneAgentFile(args: {
  ...
  const normalizedModel =
    model && model.length > 0 ? mapClaudeModelToAmawta(model) : undefined
