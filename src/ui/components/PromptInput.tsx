...
function PromptInput({
  ...
  const defaultPromptPlaceholder =
    'ask amawta to analyze, edit files, or run commands...'
  ...
  const textInputColumns = Math.max(20, columns - 12)
  ...
  return (
    <Box flexDirection="column">
      {(mode === 'bash' || modelInfo) && (
        ...
        >
          {mode === 'bash' ? (
            <Text color={theme.bashBorder}>shell cwd: {currentPwd}</Text>
          ) : (
            <Text> </Text>
          )}
          {modelInfo && (
      ...
      >
        ...
        >
          {mode === 'bash' ? (
            ...
            <Text color={isLoading ? theme.secondaryText : theme.inputBorder}>
              λ
            </Text>
          )}
        </Box>
        <Box paddingRight={1} flexGrow={1}>
          <TextInput
            ...
            placeholder={submitCount > 0 ? undefined : defaultPromptPlaceholder}
          ...
          <Box flexDirection="row" justifyContent="space-between">
            ...
            <Box justifyContent="flex-end">
              <Text dimColor wrap="truncate-end">
                {statusLine
                  ? `${statusLine} · esc interrupt · esc esc undo`
                  : 'esc interrupt · esc esc undo'}
              </Text>
            </Box>
          </Box>
