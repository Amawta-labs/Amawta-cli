      ...
      {options.slice(start, end).map((opt, idx) => {
        ...
        return (
          <Box key={opt.value} flexDirection="row">
            <Text color={isFocused ? theme.amawta : theme.secondaryText}>
...
const MANUAL_MODEL_INPUT_PROVIDERS = new Set<ProviderType>([
  'anthropic',
  'gemini',
  'adk',
  'kimi',
...
function supportsManualModelInput(provider: ProviderType): boolean {
  return MANUAL_MODEL_INPUT_PROVIDERS.has(provider)
}
...
export function ModelSelector({
  ...
  const autoSubmitApiKeyRef = useRef(false)

  const normalizeContextLengthToPreset = (value?: number): number => {
    if (!Number.isFinite(value) || (value as number) <= 0) {
      return DEFAULT_CONTEXT_LENGTH
    }
    const numericValue = Math.round(value as number)
    const exact = CONTEXT_LENGTH_OPTIONS.find(opt => opt.value === numericValue)
    if (exact) return exact.value
    let closest = CONTEXT_LENGTH_OPTIONS[0].value
    let closestDistance = Math.abs(closest - numericValue)
    for (const option of CONTEXT_LENGTH_OPTIONS) {
      const distance = Math.abs(option.value - numericValue)
  ...
  const mainMenuOptions = [
    { value: 'custom-openai', label: 'Bring Your Own OpenAI API' },
    {
      value: 'custom-anthropic',
      label: 'Bring Your Own Anthropic Messages API',
    },
    { value: 'partnerProviders', label: 'Official Partner Models →' },
    { value: 'partnerCodingPlans', label: 'Coding Specialist Models →' },
    {
      value: 'ollama',
  ...
  useEffect(() => {
    if (!apiKeyEdited && selectedProvider) {
      const envKey =
        selectedProvider === 'gemini' || selectedProvider === 'adk'
          ? process.env.GOOGLE_GENAI_API_KEY || process.env.GEMINI_API_KEY
          : process.env[selectedProvider.toUpperCase() + '_API_KEY']

      setApiKey(envKey ? String(envKey) : '')
    }
  }, [selectedProvider, apiKey, apiKeyEdited])

  useEffect(() => {
    autoSubmitApiKeyRef.current = false
  }, [selectedProvider])
  ...
  useEffect(() => {
    if (
      ...
      apiKey &&
      !apiKeyEdited &&
      !autoSubmitApiKeyRef.current
    ) {
      autoSubmitApiKeyRef.current = true
      handleApiKeySubmit(apiKey)
    }
  }, [currentScreen, selectedProvider, apiKey, apiKeyEdited])
  ...
  useEffect(() => {
    if (
      ...
      setContextLength(normalizeContextLengthToPreset(contextLength))
    }
  }, [currentScreen, contextLength])
  ...
  async function fetchModelsWithRetry() {
    ...
    const supportsManualInput = supportsManualModelInput(selectedProvider)
  ...
  async function fetchModels() {
    ...
    try {
      ...
      if (selectedProvider === 'adk') {
        const adkModels = await modelFetchers.fetchAdkModels({
          apiKey,
          setModelLoadError,
        })
        setAvailableModels(adkModels)
        navigateTo('model')
        return adkModels
      }
      ...
      for (const model of response.data) {
        ...
        fetchedModels.push({
          ...
          context_length: modelInfo?.max_input_tokens,
  ...
  function handleModelSelection(model: string) {
    ...
    setContextLength(normalizeContextLengthToPreset(modelInfo?.context_length))
  ...
  async function saveConfiguration(
    ...
    try {
      ...
      const normalizedModelName = isGoogleModelProvider(actualProvider)
        ? normalizeGeminiModelName(model)
        : model

      const existingProfile = modelManager
        .getAllConfiguredModels()
        .find(
          profile =>
            profile.isActive && profile.modelName === normalizedModelName,
        )
      if (existingProfile) {
        return existingProfile.modelName
      }
      ...
      const modelConfig = {
        ...
        modelName: normalizedModelName || actualProvider,
        baseURL: baseURL,
        apiKey: apiKey || '',
  ...
  useInput((input, key) => {
    ...
    if (currentScreen === 'apiKey' && key.tab) {
      if (supportsManualModelInput(selectedProvider)) {
        navigateTo('modelInput')
        return
      }
          ...
          <Box flexDirection="column" gap={1}>
            ...
            <Box marginTop={1}>
              <Text dimColor>
                ...
                {supportsManualModelInput(selectedProvider)
  ...
  if (currentScreen === 'modelInput') {
    ...
    } else if (selectedProvider === 'gemini' || selectedProvider === 'adk') {
      screenTitle =
        selectedProvider === 'gemini'
          ? 'Gemini Model Setup'
          : 'Google ADK Model Setup'
      description = `Enter the model name for ${modelTypeText}:`
      examples =
        'For example: "gemini-3-pro" or "gemini-3-flash" (preview IDs are also supported).'
    ...
    return (
      <Box flexDirection="column" gap={containerGap}>
        ...
        >
          ...
          <Box flexDirection="column" gap={containerGap}>
            <Text bold>
              Choose an official provider for this profile:
            </Text>
            <Box flexDirection="column" width={70}>
              <Text color={theme.secondaryText}>
                {compactLayout
                  ? 'Use a trusted provider catalog.'
                  : 'Pick from official partner catalogs to unlock their hosted models and capabilities in Amawta.'}
              </Text>
            </Box>
    ...
    return (
      <Box flexDirection="column" gap={containerGap}>
        ...
        >
          ...
          <Box flexDirection="column" gap={containerGap}>
            <Text bold>
              Choose a coding-optimized provider profile:
            </Text>
            <Box flexDirection="column" width={70}>
              <Text color={theme.secondaryText}>
                {compactLayout ? (
                  'Coding-first models from partner providers.'
                ) : (
                  <>
                    These profiles are tuned for software tasks and multi-file
                    workflows.
                    <Newline />
                    Some options may require provider-specific coding plans.
                  </>
                )}
              </Text>
            </Box>
  ...
  return (
    <ScreenContainer
      ...
      children={
        <Box flexDirection="column" gap={containerGap}>
          <Text bold>
            Choose your model source for this profile:
          </Text>
          <Box flexDirection="column" width={70}>
            <Text color={theme.secondaryText}>
              {compactLayout ? (
                'Select where Amawta should source models.'
              ) : (
                <>
                  Select the provider or custom endpoint that Amawta should use
                  for this profile.
                  <Newline />
                  This determines which model catalogs and capabilities you can
                  configure next.
                </>
              )}
            </Text>
          ...
          <Box marginTop={tightLayout ? 0 : 1}>
            <Text dimColor>
              You can update this anytime with{' '}
