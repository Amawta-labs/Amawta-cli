import builtInModels from '@constants/models'
...
type GoogleProvider = 'gemini' | 'adk'

const CURATED_GOOGLE_MODEL_NAMES = new Set([
  'gemini-3-pro-preview',
  'gemini-3-flash-preview',
])
const CURATED_GOOGLE_MODEL_KEYS = new Set(
  Array.from(CURATED_GOOGLE_MODEL_NAMES).map(name =>
    normalizeGeminiModelName(name).toLowerCase(),
  ),
)

function isCuratedGoogleModelName(name: string): boolean {
  return CURATED_GOOGLE_MODEL_KEYS.has(
    normalizeGeminiModelName(name).toLowerCase(),
  )
}

function getCuratedGoogleModels(provider: GoogleProvider): ModelInfo[] {
  const geminiCatalog = builtInModels.gemini || []
  return geminiCatalog
    .filter(model => isCuratedGoogleModelName(model.model))
    .map(model => ({
      model: toGeminiDisplayModelName(model.model),
      provider,
      max_tokens: model.max_output_tokens || model.max_tokens,
      context_length: model.max_input_tokens,
      supports_vision: !!model.supports_vision,
      supports_function_calling: !!model.supports_function_calling,
      supports_reasoning_effort: !!(model as any).supports_reasoning_effort,
    }))
}

function mergeGoogleModelLists(
  primary: ModelInfo[],
  fallback: ModelInfo[],
): ModelInfo[] {
  ...
  for (const model of [...primary, ...fallback]) {
    if (!isCuratedGoogleModelName(model.model)) {
      continue
    }
    const key = normalizeGeminiModelName(model.model).toLowerCase()
    if (!merged.has(key)) {
...
async function fetchGoogleGenAIModels({
  apiKey,
  provider,
  setModelLoadError,
}: {
  ...
  const curatedModels = getCuratedGoogleModels(provider)
  ...
  try {
    ...
    if (!resolvedApiKey) {
      throw new Error(
        'Missing Gemini API key (GOOGLE_GENAI_API_KEY or GEMINI_API_KEY).',
      )
    }
    ...
    const { models: responseModels = [] } = await response.json()

    const remoteModels = responseModels
      .filter((model: any) => {
        ...
        return {
          ...
          context_length:
            typeof model.inputTokenLimit === 'number'
              ...
              : typeof model.inputTokenLimit === 'string'
                ? Number.parseInt(model.inputTokenLimit, 10)
              : typeof model.input_token_limit === 'number'
                ? model.input_token_limit
              : typeof model.input_token_limit === 'string'
                ? Number.parseInt(model.input_token_limit, 10)
              : typeof model.contextWindow === 'number'
                ? model.contextWindow
              : typeof model.contextWindow === 'string'
                ? Number.parseInt(model.contextWindow, 10)
              : undefined,
          supports_vision: methods.includes('generateContent'),
          supports_function_calling: methods.includes('generateContent'),
        }
      })
      .filter(model => isCuratedGoogleModelName(model.model))

    return mergeGoogleModelLists(remoteModels, curatedModels)
  } catch (error) {
    ...
    if (curatedModels.length > 0) {
      setModelLoadError(
        `${message}\n\nUsing curated Gemini model list as fallback.`,
      )
...
export async function fetchAdkModels({
  apiKey,
  setModelLoadError,
}: {
  apiKey: string
...
export const __testOnly = {
  getCuratedGoogleModels,
  mergeGoogleModelLists,
}
