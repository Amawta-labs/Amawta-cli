...
export function AssistantToolUseMessage({
  ...
  const isLowEmphasisToolRun =
    resolvedName === 'DialecticalAnalysis' ||
    resolvedName === 'BaconianAnalysis' ||
    resolvedName === 'HypothesisNormalization' ||
    resolvedName === 'FalsificationPlan' ||
    resolvedName === 'ExperimentRunners'
  const theme = getTheme()
  const lowEmphasisReadableColor = theme.text === '#fff' ? '#b8b8b8' : '#555'
  const color =
    isQueued
      ? theme.secondaryText
      : isLowEmphasisToolRun
        ? lowEmphasisReadableColor
        : undefined
      ...
      <Box>
        ...
        >
          ...
          {tool.name === 'Task' && param.input ? (
            ...
            hasToolName && (
              <Text color={color} bold={!isQueued && !isLowEmphasisToolRun}>
                {userFacingToolName}
              </Text>
            )
          )}
