...
import { applyToolPermissionContextUpdates } from '@amawta-types/toolPermissionContext'
...
function getAcpSessionDir(cwd: string): string {
  return join(getConfigBaseDir(), getProjectDirSlug(cwd), 'acp-sessions')
}
...
export class AmawtaAcpAgent {
  ...
  private async handleSessionPrompt(params: unknown): Promise<Protocol.PromptResponse> {
    ...
    if (process.env.AMAWTA_ACP_ECHO === '1') {
      await this.handleAmawtaMessage(session, createAssistantMessage(promptText))
      persistAcpSessionToDisk(session)
    ...
    try {
      for await (const m of query(baseMessages, session.systemPrompt, session.context, canUseTool, {
        ...
        await this.handleAmawtaMessage(session, m)
      }
  ...
  private async handleAmawtaMessage(session: SessionState, m: Message): Promise<void> {
  ...
  private createAcpCanUseTool(session: SessionState): CanUseToolFn {
    const timeoutMs = (() => {
      const raw = process.env.AMAWTA_ACP_PERMISSION_TIMEOUT_MS
