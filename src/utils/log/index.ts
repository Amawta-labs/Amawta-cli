...
import type { LogOption, SerializedMessage } from '@amawta-types/logs'
...
function getLegacyCacheRoot(): string {
  return process.env.AMAWTA_LEGACY_CACHE_ROOT ?? paths.cache
}

function getNewLogRoot(): string {
  return process.env.AMAWTA_LOG_ROOT ?? getConfigBaseDir()
}
