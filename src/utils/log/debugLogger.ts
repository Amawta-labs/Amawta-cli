...
import type { Message } from '@amawta-types/conversation'
...
const AMAWTA_DIR = join(homedir(), '.amawta')
function getProjectDir(cwd: string): string {
  return cwd.replace(/[^a-zA-Z0-9]/g, '-')
}

const DEBUG_PATHS = {
  base: () => join(AMAWTA_DIR, getProjectDir(process.cwd()), 'debug'),
...
export function logAPIError(context: {
  ...
  const errorDir = join(AMAWTA_DIR, 'logs', 'error', 'api')
...
export function logSystemPromptConstruction(construction: {
  ...
  if (construction.amawtaContext) {
    terminalLog(`   + Amawta Context: ${construction.amawtaContext.length} chars`)
  }
...
export function diagnoseError(error: any, context?: any): ErrorDiagnosis {
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        'Check whether ESC was pressed to cancel the request',
        'Verify network stability',
        'Confirm AbortController state: isActive and signal.aborted should align',
        'Check for overlapping requests causing conflicts',
      ],
      debugSteps: [
        'Run with --debug-verbose for detailed request flow',
        'Inspect BINARY_FEEDBACK_* events in debug logs',
        'Verify REQUEST_START and REQUEST_END log pairing',
        'Check the trigger source of QUERY_ABORTED events',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        'Run /model to reconfigure the API key',
        'Check API key values in ~/.amawta configuration',
        'Verify key validity and expiration status',
        'Confirm provider configuration (anthropic/opendev/bigdream)',
      ],
      debugSteps: [
        'Review CONFIG_LOAD logs for provider and key status',
        'Run amawta doctor for environment diagnostics',
        'Inspect API_ERROR logs for exact failure reason',
        'Run amawta config to verify active configuration',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        ...
        'Confirm firewall rules are not blocking required ports',
        'Verify proxy configuration if your environment requires one',
        'Try a different network environment',
        'Validate baseURL configuration',
      ],
      debugSteps: [
        'Inspect API_REQUEST_START and related network logs',
        'Review LLM_REQUEST_ERROR details',
        'Test endpoint reachability with ping or curl',
        'Check whether enterprise proxy settings are required',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        ...
        'Confirm current user has sufficient privileges',
        'Use elevated privileges only when strictly necessary',
        'Validate tool permission settings',
      ],
      debugSteps: [
        'Review PERMISSION_* logs for decision flow',
        'Inspect filesystem permissions with ls -la',
        'Verify tool approval state',
        'Check TOOL_* debug logs for execution context',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        'Confirm the active provider matches expectations',
        'Verify response format parsing logic',
        'Account for provider-specific response differences',
        'Update response parsing code if required',
      ],
      debugSteps: [
        'Inspect LLM_CALL_DEBUG payload format',
        'Check provider config against API used at runtime',
        'Compare Anthropic and OpenAI response structures',
        'Validate formatting in logLLMInteraction',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        'Run /compact to reduce conversation history',
        'Validate auto-compaction settings',
        'Reduce single-message input size',
        'Trim non-essential context from the prompt',
      ],
      debugSteps: [
        'Inspect AUTO_COMPACT_* logs for trigger conditions',
        'Review token usage and configured thresholds',
        'Check CONTEXT_COMPRESSION logs',
        'Confirm model max-token limits',
      ],
    }
  }
  ...
  if (
    ...
    return {
      ...
      suggestions: [
        'Run amawta config and verify active settings',
        'Recreate configuration files if they are corrupted',
        'Validate JSON syntax in config files',
        'Verify required environment variables',
      ],
      debugSteps: [
        'Inspect CONFIG_LOAD and CONFIG_SAVE logs',
        'Check config file paths and filesystem permissions',
        'Validate JSON with: cat ~/.amawta/config.json | jq',
        'Review config cache behavior in debug logs',
      ],
    }
  }
  ...
  return {
    ...
    suggestions: [
      ...
      'Inspect complete error logs for more context',
      'If persistent, report the issue with reproduction details',
    ],
    debugSteps: [
      'Run with --debug-verbose for full diagnostics',
      'Inspect error.log for stack traces and context',
