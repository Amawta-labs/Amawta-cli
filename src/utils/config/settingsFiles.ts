...
function getDefaultHomeDir(): string {
  // Unit tests should never write to the real user home directory. Some CI/sandbox
  // environments mount `$HOME` read-only, which breaks settings migrations.
  if (process.env.NODE_ENV === 'test') {
    return join(process.cwd(), '.tmp-amawta-home')
  }
...
function getUserAmawtaBaseDir(options?: {
  homeDir?: string
  ...
  if (respectEnvOverride) {
    const override = normalizeOverride(
      process.env.AMAWTA_CONFIG_DIR ?? process.env.CLAUDE_CONFIG_DIR,
    )
    if (override) return override
  }
  const home = options?.homeDir ?? getDefaultHomeDir()
  return join(home, '.amawta')
}
...
export function getSettingsFileCandidates(options: {
  ...
  switch (options.destination) {
    case 'localSettings': {
      const primary = join(projectDir, '.amawta', 'settings.local.json')
      const legacy = [join(projectDir, '.claude', 'settings.local.json')]
      return { primary, legacy }
    }
    case 'projectSettings': {
      const primary = join(projectDir, '.amawta', 'settings.json')
      const legacy = [join(projectDir, '.claude', 'settings.json')]
      return { primary, legacy }
    }
    case 'userSettings': {
      const primary = join(
        getUserAmawtaBaseDir({ homeDir, respectEnvOverride }),
        'settings.json',
      )
