...
export function getGlobalConfigFilePath(): string {
  return process.env.AMAWTA_CONFIG_DIR || process.env.CLAUDE_CONFIG_DIR
    ? join(getConfigBaseDir(), 'config.json')
    : join(homedir(), CONFIG_FILE)
}

export function getMemoryDir(): string {
  return join(getConfigBaseDir(), 'memory')
}

export const CONFIG_BASE_DIR_PATH = getConfigBaseDir()
