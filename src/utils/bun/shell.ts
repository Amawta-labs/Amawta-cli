...
function maybeAnnotateMacosSandboxStderr(
  ...
  const looksLikeSandboxViolation =
    stderr.includes('AMAWTA_SANDBOX') ||
  ...
  return [
    stderr.trimEnd(),
    '',
    '[sandbox] This failure looks like a macOS sandbox denial. Adjust sandbox settings (e.g. /sandbox or .amawta/settings.json) to grant the minimal required access.',
  ].join('\n')
}
...
export function buildLinuxBwrapFilesystemArgs(options: {
  ...
  if (writeConfig) {
    ...
    if (existsSync('/tmp/amawta')) {
      args.push('--bind', '/tmp/amawta', '/tmp/amawta')
      allowedRoots.push('/tmp/amawta')
    }
...
function buildSandboxEnvAssignments(options?: {
  ...
  const env: string[] = ['SANDBOX_RUNTIME=1', 'TMPDIR=/tmp/amawta']
...
export function buildMacosSandboxExecCommand(options: {
  ...
  const logTag = 'AMAWTA_SANDBOX'

  const profileLines: string[] = [
    '(version 1)',
    `(deny default (with message "${logTag}"))`,
    '',
    '; Amawta sandbox-exec profile (reference CLI compatible)',
    '',
    '(allow process*)',
    '(allow sysctl-read)',
    '(allow mach-lookup)',
...
export class BunShell {
  ...
  private buildSandboxCmd(
    ...
    if (platform === 'darwin') {
      ...
      try {
        mkdirSync('/private/tmp/amawta', { recursive: true })
      } catch {}
