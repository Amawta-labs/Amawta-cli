...
export function isPackagedRuntime(): boolean {
  if (isTruthyEnv(process.env.AMAWTA_PACKAGED)) return true
...
export function maybeTruncateVerboseToolOutput(
  ...
  const maxLinesEnv = Number(process.env.AMAWTA_TOOL_OUTPUT_MAX_LINES ?? '')
  const maxCharsEnv = Number(process.env.AMAWTA_TOOL_OUTPUT_MAX_CHARS ?? '')
  const envOverrides = {
    maxLines:
  ...
  const fullAllowed = isTruthyEnv(process.env.AMAWTA_TOOL_OUTPUT_FULL)
