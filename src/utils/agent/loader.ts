...
function getUserConfigRoots(): string[] {
  ...
  const amawtaOverride = normalizeOverride(process.env.AMAWTA_CONFIG_DIR)

  const hasAnyOverride = Boolean(claudeOverride || amawtaOverride)
  if (hasAnyOverride) {
    return dedupeStrings([claudeOverride ?? '', amawtaOverride ?? ''])
  }

  return dedupeStrings([join(homedir(), '.claude'), join(homedir(), '.amawta')])
}
...
function findProjectAgentDirs(cwd: string): string[] {
  ...
  while (current !== home) {
    ...
    const amawtaDir = join(current, '.amawta', 'agents')
...
const BUILTIN_STATUSLINE_SETUP: AgentConfig = {
  ...
  whenToUse:
    'Set up the CLI status line command (writes to ~/.amawta/settings.json statusLine). Use when the user runs /statusline.',
...
Requirements:
- Write/update the user's ~/.amawta/settings.json and set the top-level key "statusLine" to a shell command string.
