...
function resolveRipgrepPathOrThrow(): string {
  const explicit = process.env.AMAWTA_RIPGREP_PATH
  if (explicit) {
    if (!existsSync(explicit)) {
      throw new Error(`AMAWTA_RIPGREP_PATH points to a missing file: ${explicit}`)
    }
    return explicit
  }
  ...
  if (!rgPath || !existsSync(rgPath)) {
    throw new Error(
      [
        ...
        '- Or reinstall amawta-cli (ensure dependencies are present)',
      ].join('\n'),
    )
  }
