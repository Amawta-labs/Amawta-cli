...
export type FullToolUseResult = {
  ...
  stage?:
    ...
    | 'other'
  evidence?: {
    status?: 'ready' | 'skipped' | 'unknown'
    executed?: boolean
    runsTotal?: number
    runsOk?: number
    runsFailed?: number
    toyTruth?: 'PASS' | 'FAIL' | 'INCONCLUSIVE'
    runnerContract?: 'PASS' | 'FAIL'
...
export function createProgressMessage(
  ...
  return {
    ...
    tools,
    toolUseID,
    ...(options?.persistHistory ? { persistHistory: true } : {}),
    uuid: randomUUID(),
  }
}
...
export function reorderMessages(
  ...
  function findLastIndex(
    ...
    predicate: (item: NormalizedMessage) => boolean,
  ): number {
  ...
  function isRelatedToToolUseId(
    item: NormalizedMessage,
    toolUseID: string,
  ): boolean {
    if (item.type === 'progress') return item.toolUseID === toolUseID
    if (
      item.type === 'assistant' &&
      Array.isArray(item.message.content) &&
      isToolUseLikeBlockParam(item.message.content[0]) &&
      item.message.content[0].id === toolUseID
    ) {
      return true
    }
    if (
      ...
      item.message.content[0].tool_use_id === toolUseID
    ) {
      return true
    }
    return false
  ...
  for (const message of messages) {
    ...
    if (message.type === 'progress') {
      ...
      const toolUseIndex = findLastIndex(
        ms,
        item =>
          item.type === 'assistant' &&
          Array.isArray(item.message.content) &&
      ...
      const lastProgressIndex = findLastIndex(
        ms,
        item => item.type === 'progress' && item.toolUseID === toolUseID,
      )

      const lastToolResultIndex = findLastIndex(
        ms,
        item =>
          item.type === 'user' &&
          Array.isArray(item.message.content) &&
      ...
      // Progress should render after the tool_use but before the tool_result
      // (even if the tool_result arrived before the progress message).
      const baseIndex = lastProgressIndex >= 0 ? lastProgressIndex : toolUseIndex
      let insertIndex = baseIndex >= 0 ? baseIndex + 1 : ms.length
      if (lastToolResultIndex >= 0 && insertIndex > lastToolResultIndex) {
        insertIndex = lastToolResultIndex
      }

      if (message.persistHistory === true) {
        ms.splice(insertIndex, 0, message)
        continue
      }

      const existingTransientProgressIndex = findLastIndex(
        ms,
        item =>
          item.type === 'progress' &&
          item.toolUseID === toolUseID &&
          item.persistHistory !== true,
      )
      if (existingTransientProgressIndex >= 0) {
        ms[existingTransientProgressIndex] = message
        continue
      }
    ...
    if (
      ...
      if (lastProgressIndex >= 0) {
        ms.splice(lastProgressIndex + 1, 0, message)
        continue
      }
...
export function normalizeContentFromAPI(
  ...
  const sanitizedContent = content.map(block => {
    ...
    return {
      ...block,
      text: stripInternalReminderLeakage(block.text || ''),
    }
  })

  const filteredContent = sanitizedContent.filter(
...
const INTERNAL_REMINDER_BLOCK_PATTERN =
  /<system-reminder>[\s\S]*?<\/system-reminder>/gi
const INTERNAL_TODO_CHANGE_PATTERN =
  /Your todo list has changed\.[\s\S]*?Continue on with the tasks at hand if applicable\./gi
const INTERNAL_TODO_EMPTY_PATTERN =
  /This is a reminder that your todo list is currently empty\.[\s\S]*?Again do not mention this message to the user\./gi

export function stripInternalReminderLeakage(text: string): string {
  if (!text) return ''

  return text
    .replace(INTERNAL_REMINDER_BLOCK_PATTERN, '')
    .replace(INTERNAL_TODO_CHANGE_PATTERN, '')
    .replace(INTERNAL_TODO_EMPTY_PATTERN, '')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}
