...
import { getAmawtaAgentSessionId } from './agentSessionId'
...
function resolvePersistTarget(toolUseContext: {
  ...
  return { kind: 'session', sessionId: getAmawtaAgentSessionId() }
}
...
export function appendSessionSummaryRecord(args: {
  ...
  const sessionId = args.sessionId ?? getAmawtaAgentSessionId()
  const cwd = getCwd()
...
export function appendSessionCustomTitleRecord(args: {
  ...
  if (args.sessionId === getAmawtaAgentSessionId()) {
