...
import { getSessionProjectDir } from './agentSessionLog'
...
export type AmawtaAgentSessionLogData = {
  messages: Message[]
...
export function loadAmawtaAgentSessionLogData(args: {
  cwd: string
  sessionId: string
}): AmawtaAgentSessionLogData {
...
export function loadAmawtaAgentSessionMessages(args: {
  cwd: string
  sessionId: string
}): Message[] {
  return loadAmawtaAgentSessionLogData(args).messages
}

export function findMostRecentAmawtaAgentSessionId(cwd: string): string | null {
  const projectDir = getSessionProjectDir(cwd)
