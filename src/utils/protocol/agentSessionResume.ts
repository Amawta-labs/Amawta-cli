...
export type AmawtaAgentSessionListItem = {
  sessionId: string
  slug: string | null
...
function readSessionListItemBestEffort(args: {
  filePath: string
  sessionId: string
}): Omit<AmawtaAgentSessionListItem, 'sessionId'> {
  ...
  for (const rawLine of content.split('\n')) {
    ...
    if (!branch && typeof entry.gitBranch === 'string' && entry.gitBranch.trim()) {
      branch = entry.gitBranch.trim()
    }
...
export function listAmawtaAgentSessions(args: {
  cwd: string
}): AmawtaAgentSessionListItem[] {
  const { cwd } = args
