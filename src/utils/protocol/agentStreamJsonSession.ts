...
import {
  amawtaMessageToSdkMessage,
  makeSdkResultMessage,
  type SdkMessage,
} from './agentStreamJson'
import type { AmawtaAgentStructuredStdio } from './agentStructuredStdio'
...
export async function runAmawtaAgentStreamJsonSession(args: {
  structured: AmawtaAgentStructuredStdio
  query: QueryFn
  ...
  while (true) {
    ...
    if (args.replayUserMessages) {
      const sdkUserOut = amawtaMessageToSdkMessage(userMsg, args.sessionId)
    ...
    try {
      ...
      for await (const m of args.query(
        ...
        const sdk = amawtaMessageToSdkMessage(m as any, args.sessionId)
