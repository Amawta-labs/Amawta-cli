...
} from '@amawta-types/toolPermissionContext'
...
function validatePathRestrictedCommand(
  ...
  if (hasCdInCompound && op !== 'read') {
    return {
      behavior: 'ask',
      message:
        "Commands that change directories and perform write operations require explicit approval to ensure paths are evaluated correctly. For security, Amawta cannot automatically determine the final working directory when 'cd' is used in compound commands.",
      decisionReason: {
        type: 'other',
        reason:
...
function validateOutputRedirections(
  ...
  if (hasCdInCompound && redirections.length > 0) {
    return {
      behavior: 'ask',
      message:
        "Commands that change directories and write via output redirection require explicit approval to ensure paths are evaluated correctly. For security, Amawta cannot automatically determine the final working directory when 'cd' is used in compound commands.",
      decisionReason: {
        type: 'other',
        reason:
...
function h02(args: {
  ...
  if (
    !parseBoolLikeEnv(
      process.env.AMAWTA_DISABLE_COMMAND_INJECTION_CHECK ??
