...
export function isWriteProtectedPath(
  ...
  if (normalized.endsWith('/.amawta/settings.json')) return true
  if (normalized.endsWith('/.amawta/settings.local.json')) return true
  ...
  const protectedDirs = [
    ...
    POSIX.join(projectRootPosix, '.amawta', 'commands'),
    POSIX.join(projectRootPosix, '.amawta', 'agents'),
    POSIX.join(projectRootPosix, '.amawta', 'skills'),
  ]
...
function rootPathForSource(source: ToolPermissionUpdateDestination): string {
  switch (source) {
    ...
    case 'userSettings':
      return resolveLikeCliPath(getConfigBaseDir())
    case 'policySettings':
...
export function getSpecialAllowedReadReason(args: {
  ...
  const baseDirResolved = resolveLikeCliPath(getConfigBaseDir())
