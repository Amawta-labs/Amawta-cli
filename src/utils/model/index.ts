...
export const USE_BEDROCK = !!(
  process.env.AMAWTA_USE_BEDROCK ?? process.env.CLAUDE_CODE_USE_BEDROCK
)
export const USE_VERTEX = !!(
  process.env.AMAWTA_USE_VERTEX ?? process.env.CLAUDE_CODE_USE_VERTEX
)
...
export class ModelManager {
  ...
  resolveModel(modelParam: string | ModelPointerType): ModelProfile | null {
    ...
    if (typeof modelParam === 'string') {
      const byGeminiAlias = this.findModelProfileByGeminiAlias(modelParam)
      if (byGeminiAlias && byGeminiAlias.isActive) {
        return byGeminiAlias
      }
  ...
  resolveModelWithInfo(modelParam: string | ModelPointerType): {
    ...
    if (isPointer) {
      ...
      if (!profile && typeof modelParam === 'string') {
        profile = this.findModelProfileByGeminiAlias(modelParam)
      }
      ...
      if (!profile) {
        return {
          success: false,
          profile: null,
          error: `Model '${modelParam}' not found. Use /model to add models, or run 'amawta models list' to see configured profiles.`,
        }
      }
  ...
  private resolveProviderQualifiedModel(input: string): ModelProfile | null {
    ...
    const resolvedModelOrName = isGoogleModelProvider(provider)
      ? normalizeGeminiModelName(modelOrName)
      : modelOrName

    const byModelName = providerProfiles.find(
      p =>
        p.modelName === resolvedModelOrName ||
        (isGoogleModelProvider(p.provider) &&
          normalizeGeminiModelName(p.modelName) === resolvedModelOrName),
    )
  ...
  private findModelProfileByGeminiAlias(input: string): ModelProfile | null {
    const normalizedInput = normalizeGeminiModelName(input)
    if (!normalizedInput) return null

    for (const profile of this.modelProfiles) {
      if (!isGoogleModelProvider(profile.provider)) continue
      if (normalizeGeminiModelName(profile.modelName) === normalizedInput) {
        return profile
      }
    }
