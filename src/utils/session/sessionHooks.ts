...
const HOOK_RUNTIME_STATE_KEY = '__amawtaHookRuntimeState'
...
export function updateHookTranscriptForMessages(
  ...
  const dir = join(tmpdir(), 'amawta-hooks-transcripts')
  try {
...
async function runCommandHook(args: {
  ...
  // Bun's Node.js-compat `child_process.spawn()` does not reliably deliver stdin
  // in our sandboxed environment (it can arrive as EOF with 0 bytes). Prefer
  // using Bun's native spawn when available.
  const BunGlobal = (globalThis as any).Bun
  if (BunGlobal && typeof BunGlobal.spawn === 'function') {
    let input = ''
    try {
      input = JSON.stringify(args.stdinJson)
    } catch (err) {
      return {
        exitCode: 1,
        stdout: '',
...
async function runPromptHook(args: {
  ...
  try {
    ...
    const systemPrompt = [
      'You are executing a Amawta prompt hook.',
...
export async function getSessionStartAdditionalContext(args?: {
  ...
  const envFileDir = mkdtempSync(join(tmpdir(), 'amawta-env-'))
...
export async function runPreToolUseHooks(args: {
  ...
  const hookInput: Record<string, unknown> = {
    session_id: getAmawtaAgentSessionId(),
...
export function __resetAmawtaHooksCacheForTests(): void {
  cache.clear()
