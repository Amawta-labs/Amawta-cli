...
The Custom Commands System (`src/services/customCommands.ts`) enables users to create reusable markdown-based commands that extend Amawta's functionality. Commands are discovered from `.claude/commands/` and `.amawta/commands/` directories and integrated seamlessly with the built-in command system.
...
function createCustomCommand(
  ...
  const scope = (baseDir.includes('.claude/commands') || baseDir.includes('.amawta/commands')) ? 'project' : 'user'
...
The Custom Commands System provides a powerful, flexible way to extend Amawta with user-defined commands while maintaining security, performance, and integration with the core system.