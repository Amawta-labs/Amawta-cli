...
Amawta uses a sophisticated multi-level configuration system that allows customization at global, project, and runtime levels. Configuration cascades from global defaults through project-specific settings to runtime overrides.
...
amawta config get theme
amawta config get -g modelProfiles.default.model

# Set configuration value
amawta config set theme dark
amawta config set -g autoUpdaterStatus enabled

# Remove configuration value
amawta config remove customApiKey
amawta config remove -g mcpServers.myserver

# List all configuration
amawta config list
amawta config list -g
```
...
> Anthropic environment overrides are disabled—configure Anthropic keys in Amawta settings instead.
...
The configuration system provides flexible, secure, and robust management of all Amawta settings while maintaining backward compatibility and user-friendly defaults.
